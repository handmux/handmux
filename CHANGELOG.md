...
- Markdown 预览顶部的横条改为阅读工具栏：空闲时是「朗读 / Aa 排版 / ⋯ 更多」，朗读时切换为「上一句 / 暂停 / 下一句 / 进度 / 语速 / 停止」。
- 朗读可以从任意位置开始：点正文里想听的句子即从该句接着读，无需从头播放，也不用拖进度条。
- 朗读时自动滚动到正在读的句子；手动滚动会暂停跟随，并浮出「回到朗读位置」按钮一键回到当前位置。
- 文本文件预览（txt/log/sh 等）现在同样有工具栏，可调字号。
- 字体大小、朗读语速、自动滚动跟随三项偏好会被记住，跨文档共享。
