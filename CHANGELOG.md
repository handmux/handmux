...
- Codex 回复期间可以继续发送消息：消息默认进入服务端队列并在当前回复完成后依次处理，也可编辑、删除或立刻引导当前回复；编辑时会暂停自动发送。
- 托管 Codex 对话支持原生 `/goal`：可查看、设置、编辑、暂停、继续或清除当前会话的持久任务目标。

### Changed

- 终端与对话的切换按钮改为显示点按后将进入的视图。
- 对话发送、停止或待发送操作失败时，具体原因改为显示在对话消息区域，不再挤在底部输入栏下方。
- Codex 会话状态弹层改为紧凑分组布局；占用圆环紧邻百分比，目录和会话 ID 均可点按整行复制，权限模式可按 TUI 的默认、自动审批和完全访问直接切换。
...
- 修复 App Server 已接收排队消息但发送回执丢失时，重连后可能自动重复发送同一消息的问题。
- 修复切换 Tab 后在历史消息加载完成前提前显示实时回复的问题。
- 修复服务端已确认排队后消息仍留在对话区，以及网络结果未知时临时消息一直显示“正在发送”的问题；无法确认是否送达时会明确提示检查对话，不再伪装成历史消息或误报失败。
- 修复排队消息编辑页面异常退出后整个队列可能一直暂停的问题；编辑锁改为可续期租约，断线后会自动释放并继续队列。
- 修复后启动或恢复的托管 Codex 首次接入时，可能把历史完成状态当成新通知的问题。
...
- 修复 Codex App Server 短暂波动时过早显示“无法连接”并遮住对话的问题。
- 统一 Codex 与 Claude Code 对话的首屏加载表现，使用终端同款加载提示且不再误显示工作波浪点。
- 修复 Codex 对话偶尔把内部 `AGENTS.md` instructions 当成用户消息展示，以及把真实 HTML / XML 消息误判为内部上下文的问题。
- Codex 停止任务后会保持停止中状态，直到 App Server 确认真正结束。
- 重新进入应用时直接恢复 Agent 的对话视图，不再先显示终端并延迟出现切换按钮。
- 对话中的审批和决策选项统一改为每项独占一行，并限制长选项高度，避免遮满屏幕。
- 修复拒绝 Codex 工具内的命令审批后仍显示成功对勾的问题；持久化记录明确为用户中止时显示“已拒绝”，无法确定具体结果时保持中性。
...
- 修复 Codex 对话与终端可能关联到不同会话、在对话视图执行 `/clear` 后终端仍停留在旧会话，以及连续点击停止会重复发送 Esc 的问题；托管模式不再把其他斜杠命令或交互请求回退到终端。
- 修复 Codex 托管对话中部分工具调用不显示或重启后消失的问题；消息与全部工具调用会按 Codex 原始会话记录的真实顺序恢复。
- 修复 Codex 托管对话消息可能重复、错序或在 `/clear` 后混入旧会话的问题，并补齐额外网络 / 文件权限审批。
- 修复 App Server 重连期间任务已完成时待发送队列可能停滞、失败状态丢失，以及复用窗格受旧 Hook 记录阻塞的问题。
- 修复多 Agent 协作时后台子线程可能取代当前对话的问题；`/clear` 创建的新会话会继续使用原思考强度。
