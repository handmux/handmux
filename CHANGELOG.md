...
- 语音输入改为可指定的 provider/mode 架构：保留讯飞 IAT，新增腾讯云实时语音识别 WebSocket 2.0 与停止后整段回填的一句话识别；三者均点击开始/停止，录音时在按钮内显示实时音量波形，终端聊天与 Agent 对话使用同一绿色呼吸动效，识别结果只补入起录时的光标位置而不清空已有内容，长期密钥仍只留在电脑端。

### Changed

- 语音转写改为 provider adapter 注册架构：模式、配置字段、环境注入、签名、整句识别和浏览器实时协议均由 adapter 声明；新增服务不再修改公共录音、路由、setup 或输入框流程，现有讯飞与腾讯配置保持兼容。
- 设置新增“语音转文字”板块，显示启用状态、与 `handmux setup` 一致的当前语音方案名称，以及端上保存的语气词过滤低/中/高滑块（默认中）；腾讯实时与一句话识别分别映射 `0/1/2`，讯飞明确显示不支持。
