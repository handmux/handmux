...
- Claude 后台任务通知改为简明提示，避免在对话中显示原始 XML；修复旧窗格事件阻塞其他 Claude 窗格收件箱状态更新的问题。
- 修复 Claude 对话页生成和压缩状态不显示、手动压缩后消息持续排队，以及压缩命令出现在摘要之后的问题；`/compact` 完成后保持用户消息气泡。
- 修复 Linux 上进程查询过慢导致 Agent 对话入口不出现的问题；Bridge 连接失败时保留具体拒绝原因，并区分进程校验超时与身份不匹配。
- 修复 Claude 的完成与确认提醒不触发的问题；安装及升级同步会同时配置状态与事件写入，恢复离线完成事件补发，并支持含空格或单引号的安装路径。
