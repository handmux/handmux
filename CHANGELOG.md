...
- 会话抽屉全面重做：按会话分组展示窗口，支持展开状态记忆、窗口数量、当前窗口高亮，以及新建、重命名、前移/后移、分屏管理和删除等操作。
- 重新设计会话绑定页，支持直接绑定当前可用会话，并保留原有底部绑定入口。

### Changed

- 会话与窗口切换改为先同步界面选中状态，再在后台补齐 pane 拓扑；抽屉关闭、主屏 Window tab 和当前工作区会保持同步。
- 抽屉支持从页面边缘跟手滑入/滑出、稳定的列表滚动与回弹反馈，并在命令模式和对话模式下分别处理手势，避免误触打开抽屉。
...
- 被拒绝的排队消息现在带「重新发送」，点一下就重新投递。
- 屏幕上开着授权 / 确认卡片时不再提供「立刻引导」。
- 授权、提问和复核卡片加了描边，浅色与暗色主题下都能看清边界。
...
- 文档预览新增目录：左侧滑出的大纲，可按节收起/展开；正文里的标题左侧也能折叠该节内容（标题行始终可见），箭头大小随标题级别变化。
- 文档预览新增文档内查找：工具栏放大镜点开即变成搜索行，命中高亮并显示「第几/共几」，可上一个/下一个循环跳转。
- 文档预览新增「更多」浮窗：显示文件路径、大小、修改时间、创建时间（路径可一键复制），并提供复制 Markdown 原文、字数与字符数、重新加载。
- 支持 Mermaid 图表（```mermaid 代码块渲染成图；语法写错时回退显示源码并说明原因）与 GFM 脚注（上标引用 + 文末列表，可互相跳转）。
- 每个代码块右上角新增一键复制；文档内 `#锚点` 与终端输出里的 `file.md#小节` 链接都能直接跳到对应标题；正文里的网址改为在应用内置浏览器打开。
- 记住每份文档的阅读位置；长文滚动一段距离后右下角出现「回到顶部」。
- 文本文件（txt/log/sh 等）与 Markdown 共用同一套工具栏，可调字号。

### Changed

- 打开文件与重新加载都会先显示整页「加载中」，大文件不再点了没反应；文件标签页名过长会截断显示。
- 宽表格在还能横向滑动时，边缘会有一层很轻的渐隐提示（滑到头就消失）。
- 手机终端底部留白收紧：内容下方最多保留 1 行空行（原为 3 行）。

### Fixed

- 修复 Markdown 里的本地图片显示不出来的问题（相对路径按文档所在目录解析，中文名与带空格文件名也已支持）；加载失败会就地说明原因。
- 修复 Markdown 文档以 YAML frontmatter 开头（如 README）时被渲染成乱码段落的问题。
- 修复表格没有单元格边框、引用块与四级以下标题没有任何样式的问题。
- 修复朗读偶发点了没反应的问题；朗读无法启动时会给出明确提示。
- 界面出错不再整屏变黑、只能重启应用：现在只会影响当前画面，并给出「返回 / 重新载入」的出口。
- 修复终端实时渲染在应用隐藏光标时可能与 tmux 光标错位、导致手机端把同一段内容重复显示的问题；解析器现在始终对齐 tmux 光标，隐藏时光标仍会定位、只是不显示。
