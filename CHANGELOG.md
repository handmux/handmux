...
### Added
- 通知收件箱：脚本推送（`handmux push`）的通知会存到服务端（`~/.handmux/notifications.json`，保留最近 100 条），进入一个全屏「通知记录」页翻看历史、逐条点开看详情（带 `--url` 的会在详情里给「打开链接」）、逐条删除；点手机通知直接深链到那条消息的详情页。有未读时设置齿轮亮红点，点开某条即标该条已读。只记手动脚本推送，不含自动的会话状态提醒。
