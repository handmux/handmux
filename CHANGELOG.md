...
- Codex 对话将原生 Goal 的设置与终态显示为可恢复的对话卡片：设置或重新开始会创建全新的 active Goal，立即在右侧显示用户侧卡片并启动原生自动续跑；Codex 终态反馈留在对应回合并靠左，统一使用底部半屏详情和 iOS 风格单行等分操作，修复 Portal 内清除确认被遮挡及返回栈异步关闭的问题；终态目标可修改后重新开始，仅进行中或暂停的 Goal 常驻在实时任务列表下方。
...
- Terminal 的自动聚焦、页面键盘转发、失焦和关闭后恢复现在统一由 Overlay Stack 驱动，App 不再手工枚举各弹窗状态。
...
- 修复设置 Goal 后即时卡片与持久化记录重复显示的问题；目标卡片会随正文扩展到三行，超出后再省略，并保留舒适的上下间距。
...
- Codex 实时回复现在使用 Server 统一投影的有序事件、可寻址的最近页快照和单一消息状态；首次装载后不再由独立 HTTP 轮询改写当前消息，手机短暂断网或切到后台后会从最后确认的游标继续补拉，rollout 最终内容也会按同一消息 ID 原位替换。
