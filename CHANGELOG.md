...
- 被拒绝的排队消息现在带「重新发送」，点一下重新投递；被拒的消息本来就不会自动重试，以前只能打开编辑再保存绕过，连续被拒时就会觉得「怎么发都发不出去」。
- 屏幕上开着授权 / 确认卡片时不再提供「立刻引导」，避免消息文字被写进那张卡片。
- 授权、提问和复核卡片加了描边：卡片底色和背后的对话底色太接近，浅色与暗色主题下都看不清边界。
