...
- 朗读可以从任意位置开始：播放中点正文里想听的句子，即从该句接着读，不用从头再来。
- 朗读时自动滚动到正在读的句子；手动滚动会暂停跟随，并浮出「回到朗读位置」按钮一键回到当前位置。
- 文本文件预览（txt/log/sh 等）现在也能调字号（与 Markdown 同一工具栏）。
- 朗读现在也会读代码块（按行读），不再跳过；行内代码会连同所在句子一起读。
- 预览工具栏改为常驻顶部并加不透明底色——内容从下方滑过，不再透进工具栏。
- 预览工具栏新增「更多」，点开是一个小浮窗，显示文件路径、大小、修改时间、创建时间，路径可一键复制。
- 新增目录：工具栏最左侧按钮点开，从左侧滑出文档大纲（按标题层级缩进），点标题跳转到该处。
- 新增文档内查找：工具栏放大镜点开即变成搜索行，输入即时高亮全部命中，显示「第几/共几」，可上一个/下一个循环跳转；关闭后正文完好复原。纯文本文件同样可搜。
- 「更多」里新增重新加载：Agent 改写文件后可一键重读，结果用一条轻提示说明「内容已更新」或「内容无变化」。
- 记住每份文档的阅读位置：切走再回来会回到上次读到的地方。
- 每个代码块右上角新增一键复制按钮。
- 「更多」里新增复制 Markdown 原文、字数与字符数。
- 文档内的 `#锚点` 链接现在可以跳转；正文里的网址改为在应用内置浏览器打开，不再跳出预览。
- 终端输出（以及会话气泡）里的文件链接支持带锚点：`docs/notes.md#小节` 会打开该文件并直接跳到对应标题；锚点写错时会提示「没有找到标题」，不再毫无反应。
- 目录支持按节收起/展开（长文档的目录不再一屏放不下）。
- 正文里的标题也能收起：标题左侧的箭头可折叠该节的内容（**标题行本身始终可见**，方便继续按小节展开）；箭头大小随标题级别变化；收起的部分不会被朗读、也搜不到。
- 长篇文档滚动一段距离后，右下角出现「回到顶部」。
- 支持 Mermaid 图表：```mermaid 代码块会渲染成图；语法写错时回退显示源码并说明原因。
- 打开文档后会在后台预热图表组件（每个版本只下载一次），看到图表时通常已就绪；不读文档的场景不产生这份流量。

### Changed

- 打开较大的文件不再卡住没反应：现在会先打开预览并显示「加载中」，内容到达后自动出现。
- 重新加载的反馈改为「整页加载中」——点完立刻显示加载状态，加载完成回到正文；不再额外弹提示条。
- 文件标签页的名字过长时会截断显示（省略号），不再一个长文件名占满整条标签栏。
- 宽表格在还能横向滑动时，边缘会有一层很轻的渐隐提示（滑到头就消失）。
- 代码块的复制按钮改小并更贴右上角（点击热区不变）。
- 手机终端底部留白收紧：内容下方最多保留 1 行空行（原为 3 行），内容更贴近屏幕底部。

### Fixed

- 修复打开文档时偶发「整屏变黑、什么都点不了、返回也无效，只能重启应用」的问题：这类崩溃会让整个界面被卸载，现在出错只会影响当前预览并给出「返回 / 重新载入」的出口；同时修掉了一个会在文档类型变化时崩溃的隐患与一处可能抛异常的解码。
- 修复终端实时渲染在应用隐藏光标时可能与 tmux 光标错位、导致手机端把同一段内容重复显示（旧内容不被覆盖）的问题；解析器现在始终对齐 tmux 光标，隐藏时光标仍会定位、只是不显示。
- 修复点开查找时工具栏变高、把正文标题往下推几像素的问题（现在三种工具栏状态高度一致）。
- 修复朗读点播放没有反应的问题：iOS 只在用户点击的当下才开始朗读，之前等待语音列表的做法会让播放被系统直接丢弃。
- 朗读无法启动时不再静默：现在会明确提示，而不是让点击看起来像没反应。
- 修复 Markdown 里的本地图片显示不出来的问题（相对路径按文档所在目录解析，并通过带认证的请求加载）。
- 修复中文名/带空格文件名的图片加载不出来的问题（Markdown 会把这类路径做百分号编码，之前被重复编码）。
- 图片加载中的占位从大块深色方块改成细骨架条：不再像裂图，也不会在图片出现时把页面顶一下；命中缓存或未改动时不再闪一下占位。
- 加载失败或图片超过 50MB 会就地给出具体原因，不再只显示裂图。
- 修复脚注（`[^1]` + 定义）渲染错误的问题：以前引用会被当成链接、脚注定义整行丢失；现在渲染为上标引用 + 文末脚注列表，可互相跳转。
...
- 修复会话切换短暂过渡时显示红色"无法连接 Codex 会话"的问题；过渡期间现在复用统一的加载中状态。
- 修复 Linux 上长驻 Claude 会话在终端自动更新删掉旧版本文件后被识别不到的问题；进程仍在运行时应保持识别。
- 修复连接反复重连时内部会话通道无限增长、拖慢服务的问题：不再可消费的通道会被回收，且总数量有上限。
- 修复 Hook 源文件在投递失败时无限堆积的问题：按时间与数量保留，超出的最旧文件会被清理。
- 修复 WSL2 上宿主时钟步进导致 Claude 对话入口反复消失的问题（issue #6）：进程启动时间改用不受时钟调整影响的取值。
- 修复 Linux 上 Claude 原生状态（进行中/空闲/等待）恒为未知的问题：按各平台的真实取值口径比对。
...
- 优化未开启可信设备保护时的认证与安全页面：仅显示状态和一键开启入口，并明确开启后的设备访问范围与 CLI 关闭方式。
- 修复从旧版本升级后误启用可信设备与可信访问地址限制、导致原有 Token 无法直接进入的问题。
...
- 修复 Claude 停止后仍显示运行中、后续消息持续排队，以及下一条消息与终端恢复的旧输入拼接的问题；对话页停止与终端 Escape 保持一致。
- 修复切回对话时模型或 Goal 弹窗重复打开，以及打开请求串到其他会话的问题。
- Claude 后台任务通知改为简明提示，避免在对话中显示原始 XML；修复旧窗格事件阻塞其他 Claude 窗格收件箱状态更新的问题，恢复后只显示最新状态，历史完成不再逐条提醒。
- 修复 Claude 对话页生成和压缩状态不显示、手动压缩后消息持续排队，以及压缩命令出现在摘要之后的问题；`/compact` 完成后保持用户消息气泡，并修复 `/model` 等原生命令完成后后续消息持续排队、对话页提交选择后原生选择器未响应的问题。
- 修复 Linux 上进程查询过慢导致 Agent 对话入口不出现的问题；Bridge 连接失败时保留具体拒绝原因，并区分进程校验超时与身份不匹配。
- 修复 Claude 的完成与确认提醒不触发的问题；安装及升级同步会同时配置状态与事件写入，支持离线恢复时提醒最新完成状态，并支持含空格或单引号的安装路径。
...
- 将对话页“已发送”临时占位的保留上限从 10 秒延长到 2 分钟，减少“立刻引导”后等待工具调用结束时消息先消失、再出现的空档；正式消息到达后立即交接。
...
- 设置中将终端字号、终端传输与键盘模式归入“终端”，终端字号在对话页也可调整；新增独立保存的“对话字体大小”，可在 10–20px 之间以 1px 为步进调整消息正文及其中的 Markdown 与代码，不影响工具详情与其他界面元素。

### Fixed

- 修复 compact 期间发送结果不确定、重发成功后“已发送”占位长期残留的问题；优先按正式消息身份交接，成功占位最多保留 10 秒，切换窗口后丢弃临时记录，不影响真实排队消息与对话历史。
...
- 修复直接运行或通过原生 `codex resume` 恢复的 Codex 无法进入对话接管页的问题；接管前核实当前进程与准确会话，接管中断后可在原窗格安全继续，无法确认时提供恢复命令复制和终端指引，不按目录猜测会话。等待 30 秒后提醒检查终端，短暂网络异常不误判为没有 Codex。
...
- `handmux setup` 会自动去掉语音字段的前后空格，并在首次配置、切换方案或修改字段后直接向服务商验证；关闭语音只停用功能并保留所选方案与全部凭据，重新开启时可直接沿用，旧配置仍默认开启。
- 设置新增“语音转文字”板块：开启后显示当前识别方案，并提供端上保存的语气词过滤低/中/高滑块（默认中）；腾讯实时与一句话识别支持该选项，讯飞明确显示不支持，未开启时则引导通过 `handmux setup` 配置。

### Fixed

- 修复实时终端连续输入英文、中文或使用方向键后，偶发同时显示两个光标或光标停在上一字符的问题；现在文字顺序正确，光标只在正确位置显示一次。
- 修复语音录音或云端识别失败后输入区没有任何错误提示的问题；现在会短暂显示浏览器麦克风权限、安全地址、网络或服务商配置的对应处理方式，未识别出文字时也会明确提示重试。
- 修复 Codex 上下文压缩时，内部摘要会短暂闪入对话正文的问题。
