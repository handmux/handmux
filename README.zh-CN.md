...
- **想法 · 随想随记**——不错过任何点子：每窗口一份想法清单，语音支持讯飞/腾讯实时听写与腾讯一句话识别；都用点击开始、再次点击结束，录音时显示同样的实时音量波形，终端聊天与 Agent 对话使用同一说话动效，识别文字只补入而不会清空已有草稿。腾讯一句话识别所用的 SecretId 需具备 CAM 权限 `asr:SentenceRecognition`。
