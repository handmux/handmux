...
export function createDeviceAuthRouter({ service, resolveOrigin, resolvePublicUrl, previewDomain }: {
  ...
  const status = (req: Request, res: Response): void => {
    ...
    res.json({ mode: service.mode, tokenEnabled: true, tokenAuthenticated: !!token,
      ...
      // A recovered candidate gets Set-Cookie, but cannot authorize business requests until
      // the browser sends that formal cookie back. This also works after pairing memory is lost.
      sessionPending: !!principal && !formalPrincipal,
      requiresTrustedDevice: service.requiresTrustedDevice && !formalPrincipal, publicUrl: advertisedOrigin(), previewDomain: previewOrigin(), trustedOrigins: service.trustedOrigins, authenticated: !!formalPrincipal,
      currentDeviceId: formalPrincipal && !service.isTokenPrincipal(formalPrincipal) ? formalPrincipal.deviceId : null, ...(pairing ? { pairing } : {}), serverTime: Date.now() });
  };
