...
export default {
  ...
  'auth.manageHint': 'Mode changes apply after restart. Manage devices with handmux auth add/list/edit/revoke. For better transport protection, use HTTPS whenever possible.',
  'auth.access': 'Device authorization: open the address, then run handmux auth add on this computer to approve your browser.',
  'auth.warning': '⚠ Note: a leaked fixed Token can be reused. Trusted-device authorization in handmux setup is recommended. For better credential protection, use HTTPS whenever possible.',
