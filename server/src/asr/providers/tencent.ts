...
import type { TencentAsrConfig, VoiceProviderAdapter } from '../providerRegistry.js';
...
export const tencentAdapter: VoiceProviderAdapter = {
  ...
  createStreamingSession: (config) => {
    ...
    return {
      ...
      ...buildTencentAsrSignedUrl(config as TencentAsrConfig),
    };
  },
  recognizeSentence: (config, audio) => {
    if (config.provider !== 'tencent') throw new Error('tencent adapter received another provider config');
