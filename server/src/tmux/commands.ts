...
export async function openPaneOutputCapture(paneId: string): Promise<TmuxPaneOutputCapture> {
  if (!isPaneId(paneId)) throw new TypeError('invalid pane id');
  const capture = new TmuxPaneOutputCapture({ pane: paneId, session: await paneSession(paneId) });
  try {
    await capture.start();
    return capture;
  } catch (error) {
