...
import type { DocType } from './docPath.js';
...
export function encodeCwdKey(cwd: unknown): string {
...
export interface DocNotModified {
  name: string;
  type: DocType;
  mtimeMs: number;
  notModified: true;
...
export function createDocs({ home, extraRoots = [], maxDownloadBytes = MAX_TRANSFER_BYTES }: DocsOptions) {
  ...
  async function resolveUploadDir(rawDir: unknown): Promise<RealPathResult> {
  ...
  async function resolveCwd(rawDir: unknown): Promise<RealPathResult> {
    if (typeof rawDir !== 'string' || rawDir[0] !== '/') return { error: 'not absolute', status: 400 };
    const rh = await realHomeP;
    let real;
    try { real = await fs.realpath(rawDir); }
  ...
  async function resolveBrowseDir(rawDir: unknown): Promise<RealPathResult> {
    if (typeof rawDir !== 'string' || rawDir[0] !== '/') return { error: 'not absolute', status: 400 };
    let real;
    try { real = await fs.realpath(rawDir); }
    catch { return { error: 'not found', status: 404 }; }
  ...
  async function makeDir(parentRaw: unknown, name: unknown): Promise<RealPathResult> {
    ...
    try {
      ...
      if (errorCode(e) === 'EEXIST') return { error: 'exists', status: 409 };
