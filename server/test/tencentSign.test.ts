...
describe('Tencent real-time ASR v2 signing', () => {
  ...
  it('allows all Tencent filler-word filtering levels', () => {
    for (const filterModal of ['0', '1', '2'] as const) {
      const result = buildTencentAsrSignedUrl({
        appId: '123456', secretId: 'ID', secretKey: 'KEY', filterModal,
        timestamp: 1000, expired: 1300, nonce: 42, voiceId: 'voice-1',
      });
      expect(new URL(result.url).searchParams.get('filter_modal')).toBe(filterModal);
    }
  });
});
