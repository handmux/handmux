...
describe('Claude Hook → LocalAgentBridge → Inbox vertical slice', () => {
  ...
  it.each(['session', 'process'])('keeps pending ACKs safe across a %s replacement', async (replacement) => {
    ...
    try {
      ...
      // The old ACK is deliberately held. Establish that its source is still queued before
      // releasing it; after release, the replacement event may be acknowledged immediately.
      await vi.waitFor(() => expect(sourceFiles().length).toBeGreaterThanOrEqual(1));
      release();
      await vi.waitFor(() => expect(sourceFiles().length).toBeLessThanOrEqual(1));
