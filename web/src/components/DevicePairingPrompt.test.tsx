...
describe('device authorization flow', () => {
  it.each([true, false])('explains rejected formal cookies and retries without creating another pairing (pairing present: %s)', async hasPairing => {
    server = { mode: 'trusted-device', tokenEnabled: true, tokenAuthenticated: true, authenticated: false,
      currentDeviceId: null, sessionPending: true, serverTime: Date.now(),
      ...(hasPairing ? { pairing: { id: 'pair_1', state: 'authorized' as const, expiresAt: Date.now() + 60000 } } : {}),
    };
    const onSaved = vi.fn();
    render(<DevicePairingPrompt onSaved={onSaved} />); await flush();
    expect(screen.getByText(t('devices.cookieRequired'))).toBeTruthy();
    expect(screen.queryByRole('button', { name: t('auth.requestNewCode') })).toBeNull();
    await tick(3000);
    expect(fetcher.mock.calls.every((call: unknown[]) => (call[1] as RequestInit).method === 'GET')).toBe(true);
    expect(onSaved).not.toHaveBeenCalled();
    server = { ...server, sessionPending: false, authenticated: true, currentDeviceId: 'dev_test' };
    fireEvent.click(screen.getByRole('button', { name: t('auth.retry') })); await flush();
  ...
  it('presents CLI and authorized-device approval as two alternative methods', async () => {
    ...
    expect(screen.getByText(t('auth.cliMethodTitle'))).toBeTruthy();
...
describe('mode bootstrap', () => {
  ...
  it('never mounts business UI when an old device session is present on an untrusted origin', async () => {
    fetcher.mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({
      ...server, authenticated: true, currentDeviceId: 'dev_old', originTrusted: false, serverTime: Date.now(),
    }) });
    render(<AuthBootstrap><div>Business UI</div></AuthBootstrap>); await flush();
    expect(screen.getByRole('heading', { name: t('auth.originRejectedTitle') })).toBeTruthy();
    expect(screen.queryByText('Business UI')).toBeNull();
