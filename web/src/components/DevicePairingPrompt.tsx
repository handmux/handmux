...
export default function DevicePairingPrompt({ onSaved, onSwitch }: { onSaved: () => void; onSwitch?: () => void }) {
  ...
  const accept = useCallback((next: AuthStatus) => {
    ...
    // Keep the global compatibility-token switch in sync while the pairing page polls.
    // `mode` is always trusted-device; tokenEnabled is an independent capability.
    if (next.mode === 'trusted-device' || next.authenticated) {
      applyAuthStatus(next);
      // A candidate cookie can authenticate status before the formal session cookie is
      // accepted. Only leave the gate once the server reports a real device session.
      if (next.authenticated && next.currentDeviceId) saved.current();
    }
  }, []);
