...
export default function DevicePairingPrompt({ onSaved }: { onSaved: () => void }) {
  ...
  if (status.sessionPending) return <AuthFrame title={t('auth.deviceRequired')} showHelp={false}>
    <section className="token-prompt pairing-prompt" aria-live="polite">
      <p>{t('devices.cookieRequired')}</p>
      {error && <p className="auth-error" role="alert">{error}</p>}
      <button type="button" className="auth-primary" disabled={busy} onClick={() => { void request('GET'); }}>{t('auth.retry')}</button>
    </section>
  </AuthFrame>;
      ...
      {displayCode && <>
        ...
        <div className="pairing-method-list">
          ...
          <div className="pairing-method-card">
            ...
            <p>{t('auth.authorizeWithCli')} <code>handmux auth device add</code> {t('auth.cliAfterCode')}</p>
          </div>
          <div className="pairing-method-card">
