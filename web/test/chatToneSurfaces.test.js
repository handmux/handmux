...
describe('chat tone overlay surfaces', () => {
  ...
  it.each(['.tool-sheet', '.cc-context-popover', '.agent-model-menu', '.conversation-goal-menu'])(
  ...
  it('keeps the model menu footer on the same opaque surface', () => {
    expect(cssRule('.agent-model-footer')).toMatch(/background:\s*var\(--ct-overlay\)/);
    expect(cssRule('.conversation-goal-actions')).toMatch(/background:\s*var\(--ct-overlay\)/);
  });
});
