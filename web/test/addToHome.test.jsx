...
describe('AddToHome', () => {
  ...
  it('iOS Safari shows the three-step guide, no install button', () => {
    setNav({ ua: UA.iosSafari });
    render();
    expect(card()).not.toBeNull();
    expect(container.querySelectorAll('.a2hs-steps li').length).toBe(3);
    expect(container.querySelector('.a2hs-install')).toBeNull();
  });

  it('iOS non-Safari points the user at Safari instead of showing steps', () => {
    setNav({ ua: UA.iosChrome });
    render();
    expect(container.querySelector('.a2hs-note')).not.toBeNull();
    expect(container.querySelector('.a2hs-steps')).toBeNull();
  });
  ...
  it('Android with a captured prompt offers one-tap install and calls prompt()', async () => {
    ...
    const btn = container.querySelector('.a2hs-install');
  ...
  it('dismissing remembers it and removes the sheet', () => {
    setNav({ ua: UA.iosSafari });
    render();
    click(container.querySelector('.a2hs-later'));
    expect(card()).toBeNull();
