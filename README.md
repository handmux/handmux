...
- **Chat view (experimental)** — read and drive Claude Code / Codex with Markdown bubbles and tool cards; start Codex with `handmux codex` to synchronize messages and controls through App Server, with a live context ring that opens the current session status.
