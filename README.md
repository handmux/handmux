...
- **Ideas — catch every one** — a thought the moment it strikes: a per-window idea/to-do list with click-to-toggle voice input—iFlytek/Tencent live dictation or Tencent sentence recognition after stopping—and one-tap insertion into the prompt. Every recording mode shows the same live level waveform, and recognized text is inserted without clearing the existing draft. Tencent sentence recognition requires the SecretId to have the CAM action `asr:SentenceRecognition`.
